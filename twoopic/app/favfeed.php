...
$ke=mysqli_query($con,"select * from users where Username='$_POST[cuser]' and App='$_POST[key]'");
...
if(!empty($photoonly)){
$y="select * from status where media!='' and direct='' and sharing=''order by id desc ";
$yy=mysqli_query($con,$y);
$numm=mysqli_num_rows($yy);
}

if(!empty($talkonly)){
$y="select * from status where talk!='' and direct='' and sharing='' order by id desc ";
$yy=mysqli_query($con,$y);
$numm=mysqli_num_rows($yy);
}
...
$y="select * from status where direct='' and sharing='' order by id desc ";
$yy=mysqli_query($con,$y);
$numm=mysqli_num_rows($yy);
...
$chkk=mysqli_query($con,"select * from favorites where Username='$current'");
...
$pro=preg_replace('@((https?://)?([-\w]+\.[-\w\.]+)+\w(:\d+)?(/([-\w/_\.]*(\?\S+)?)?)*)@','<a rel="external" style="color:blue" href="http://$1">two.pic/'.rand().'</a>',$clea);
...
$prop=preg_replace('@((https?://)?([-\w]+\.[-\w\.]+)+\w(:\d+)?(/([-\w/_\.]*(\?\S+)?)?)*)@','<a rel="external" style="color:blue" href="http://$1">two.pic/'.rand().'</a>',$cleae);
...
<img src="http://twoopic.nanoxcorp.com/sharedmedia/'.$talk[media].'" style="width:100%;height:auto"></a>';
}
...
<img src="http://twoopic.nanoxcorp.com/sharedmedia/'.$feed[media].'" style="width:100%;height:auto;max-height:300px;max-width:500px;border-radius:4px"></a>';
...
<a  href="topic.html?id='.$ide.'&user='.$feed[username].'&cuser='.$current.'&key='.$_POST[key].'" rel="external" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-family:"Century Gothic"" class="metro" rel="external">
<b>View Post</b></a> 
...
<a href="topic.html?id='.$ide.'&user='.$feed[username].'&cuser='.$current.'&key='.$_POST[key].'" rel="external" class="comment" user="'.$ide.'" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:#3b5998"><i  class="icon-comments-4 on-left"></i><span>('.$replies.')</span>
</a>
</td>';
...
<a target="me" class="like" user="'.$ide.'" href="#" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:#3b5998"><i class="icon-thumbs-up on-left"></i>(<span>'.$likes.'</span>)</a>
...
<a target="me" class="unlike" user="'.$ide.'" href="#" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:green"><i class="icon-thumbs-up on-left"></i>(<span>'.$likes.'</span>)</a>
...
<a href="#"  class="favorite" user="'.$ide.'" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:#3b5998"><i class="icon-star-3 on-left"></i>(<span>'.$favno.'</span>)</a>
</td>';}else{
echo '<td valign="top" class="metro">
<a href="#"  class="unfavorite" user="'.$ide.'" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:green"><i class="icon-star-3 on-left"></i>(<span>'.$favno.'</span>)</a>
</td>';

}

echo '<td valign="top" class="metro">
<a href="#" class= "share" user="'.$ide.'" data-role="button" data-mini="true" data-inline="true" data-theme="c" style="font-size:15px;color:#3b5998"><i class="icon-share on-left"></i>(<span>'.$shares.'</span>)</a>
</td></div>
</tr></table>';
