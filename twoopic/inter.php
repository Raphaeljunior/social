...
$y="select * from mentions where userid='$row[ID]' order by id desc";
$yy=mysqli_query($con,$y);
while($men=mysqli_fetch_array($yy)){

$yd="select * from status where topicid='$men[postid]' or talkid='$men[postid]' order by id desc limit 1 ";
$yyd=mysqli_query($con,$yd);
echo '<div style="backgound:#d4d4d4;width:100%;height:auto">';
while($feed=mysqli_fetch_array($yyd)){
